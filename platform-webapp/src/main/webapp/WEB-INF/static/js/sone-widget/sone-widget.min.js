...
(function($) {
	...
	function init($target) {
		...
		changeValue($target, findSelectedValue(data, ""), true);
		...
		function createCombo() {
			...
			if (options.titleWidth > 0) {
				$prefix.width(options.titleWidth);
			}
	...
	function menuItemClick($target, $item) {
		...
		if (!options.multiple) {
			...
			if (options.onSelected) {
				...
				var result = options.onSelected(oldValue, itemData);
				if (result == false) {
					hideSubPopFrom($target, 0);
					return;
				}
		...
		// 多选处理
			...
			if (options.onSelected) {
				result = options.onSelected(oldValue, newValue);
			}
	 ...
	 */
	function changeValue($target, newValue, initFlg) {

		var oldValue = $target.data("selectedValue");
		var options = $target.data('comboMenu').options;

		if (!initFlg && options.onChanged && oldValue != newValue) {
	...
	$.fn.comboMenu.methods = {
		...
		setValue : function($target, value) {
			...
			// 多选处理，如果是多选，在找到选择值的同时，还要给该值设上selected=1（选择），并处理它们的父节点到祖先的节点状态
				...
				if (options.onChanged) {
					options.onChanged(oldValue, selectedValue);
				}

				$target.data("selectedValue", selectedValue);
...
$.widget("ui.demandTable", {
	...
	_create : function() {
		...
		if (this.options.data != null) {
			scope.createTemplate(this.processData(this.options.data));
			...
			$.ajax({
						...
						success : function(mydata) {
							...
							scope.createTemplate(scope.processData(mydata));
						},
						error : function(data) {
							alert(data);
						}
	...
	processData:function(rawData){
	    var scope=this;
		var data={};
		data.increamentRatio=[];
		data.keySearch=[];
		//process the raw data
		var base=rawData.demandDegree[0].degree;
		if(base==0){
			base=1;
		}
		for(var i=0;i<rawData.demandDegree.length;i++){
			var child=rawData.demandDegree[i];
			var n=child.degree/base*100;
			var b=child.degree/base;
			if(n<1){
				n=2;
			}
			var keyObj={
			...
			data.keySearch.push(keyObj);
			data.increamentRatio.push(increamentObj);
		}
		//排序
		data.increamentRatio.sort(scope.combynumber);
		return data;
	...
	createTemplate : function(mydata) {
		...
		var tplStr = '<div class="tabConts hotWords" >'
				...
				+ '<p style="color: #999; margin-bottom: 5px; padding: 5px 0 10px 0;">热词排行榜</p>'
				+ '<table id="leftTable" class="listN1" style="width: 97%;"></table>'
				+ '</div>'
				+ '<div style="float: right; width: 46%;">'
				+ '<p style="color: #999; margin-bottom: 5px; padding: 5px 0 10px 0;">热词飙升榜</p>'
				+ '<table id="rightTable" class="listN1" style="width: 97%;"></table>'
				+ '</div>' + '</div>' + '</div>';
		var tpl = $(tplStr);
		var leftData = mydata.keySearch;
		var leftTable = '<colgroup>' + '<col>' + '<col>'+ '<col>' + '<col width="30">'
		...
		$.each(leftData, function(k, v) {
					...
					leftTable += '<tr>'
							...
							+ '</div></td>' + '<td>'+v.degree+'</td>'+'<td>热度</td>'
							+ '<td><div class="psBox">'
		...
		$.each(rightData, function(k, v) {
			var n=k+1;
			var upDownClass="fa fa-arrow-up";
			if(v.number <0){
			   upDownClass="fa fa-arrow-down";
			}
			rightTable += '<tr>'
					+ '<td class="rank">'
					+ n
					...
					+ '<a class="icons search" href="javascript:;;"></a>'
					+ '</div></td>'
					+ '<td class="txr"><i  class="'+upDownClass+'"></i><span>'+v.number+'%&nbsp;</span></td>'
					+ '</tr>';
		});
...
$(function() {
	window.Modal = function() {
		...
		// 定义模板
	    var tplStr='<div class="ycf-alert" class="modal">'+
        ...
        $(".modal-dialog").css( { position : 'absolute', 'top' : top , left : left} ).show();  
		}
...
/*******************************************************************************
 * 一个基于bootstrap的modal插件。 利用postMessage提供了跨域的解决。
 * 如果某个Iframe使用$.al.alert(...)请求弹出modal时，
 * 该插件会一直向上冒泡请求以寻求最顶层的那个Iframe去完成这个“职责”并设置相关回调。
 * 
 * 请尽可能地进行极端情况下的测试。
 * 
 * @author jiali1
...
$(function(jq) {
	var al = {};
	var tplStr = "<div class=\"modal fade\" id=\"jdConfirm\" role=\"dialog\" aria-labelledby=\"jdModalLabel\" aria-hidden=\"true\">\r\n"
			+ "	<div class=\"modal-dialog  modal-sm\">\r\n"
			+ "		<input id=\"jdAlertDimissType\" type=\"hidden\" /> <input id=\"jdAlertIsOnShow\" type=\"hidden\" />\r\n"
			+ "		<div class=\"modal-content\">\r\n"
			+ "			<div class=\"modal-header\">\r\n"
			+ "				<b class=\"modal-title fa fa-exclamation-circle\" id=\"jdAlertTitle\">警告</b>\r\n"
			+ "			</div>\r\n"
			+ "			<div class=\"modal-body \">\r\n"
			+ "				<p id=\"jdAlertContent\">请确认您的操作</p>\r\n"
			+ "			</div>\r\n"
			+ "			<div class=\"modal-footer\">\r\n"
			+ "				<button type=\"button\" class=\"btn btn-primary\" id=\"jdAlertAccept\">确定</button>\r\n"
			+ "				<button type=\"button\" class=\"btn btn-default\" data-dismiss=\"modal\" id=\"jdAlertCancel\">取消</button>\r\n"
			+ "			</div>\r\n" + "		</div>\r\n" + "	</div>\r\n" + "</div>";
	var init = function() {
		// 定义Iframe所发出的消息的handler
		var handler = function(e) {
			...
			var opt = data.opt;
			if (data.type == "alert") {
				$.al.alert(opt.title, opt.content, function() {
					var msg = {};
					msg.type = 'alertResponse';
					msg.result = true;
					// 发送消息到子窗口
					e.source.postMessage(JSON.stringify(msg), '*');
				});
			} else if (data.type == "confirm") {
				$.al.confirm(opt.title, opt.content, function(result) {
					var msg = {};
					msg.type = 'alertResponse';
					msg.result = result;
					// 发送消息到子窗口
	...
	var showAlert = function(title, content, accept) {
		// 如果当前window不处于top（在Iframe中），则通知上一级的window弹出alert
		// 这个特性使得：1，消息会冒泡传递，直到最顶层的window；
		if (top != window) {
			// 根据postMessage API发送消息。
			var msg = {};
			msg.type = "alert";
			msg.opt = {};
			msg.opt.title = title;
			msg.opt.content = content;
			var handler = function(e) {
				e = jQuery.parseJSON(e.data);
				if (e.type == 'alertResponse') {
					callBack(e.result);
				}
			};
	 ...
	 * 逻辑同showAlert。
	 **************************************************************************/
	var showConfirm = function(title, content, accept) {
		if (top != window) {
			var msg = {};
			msg.type = "confirm";
			msg.opt = {};
			...
			data.content = content;
			data.accept = accept;
			data.cancel = function(alertResult) {
				accept(false);
			};
			showOptions(data);
		}
	...
	/**
	 * 如果data.accept或data.cancel有且只有一个为空，则会去掉“取消”按钮
	 */
	var showOptions = function(data) {
		// 调用父窗口的jquery
		var modalObj = $("#jdConfirm");

		if (modalObj == null || modalObj.length == 0) {
			tplStr = $(tplStr);
			var defLoc = 100;// 默认的弹出位置

			var topLoc = -1;
			if ($("#header").length != 0) {
				topLoc = $("#header").outerHeight(true) * 1.5;
			}
			if (topLoc <= 0) {
				topLoc = defLoc;
			}
			...
			$("body").append(tplStr);
			modalObj = $("#jdConfirm");
		}// todo 也可以设置为每次弹出都清除上次的dom 元素的“模式”
		else if ($("#jdAlertIsOnShow").attr("value") == "show") {
			$("#jdConfirm").remove();
			showOptions(data);
			return;
		}
		$("#jdAlertDimissType").attr("value", "cancel");
		modalObj.unbind();
		$("#jdAlertAccept").unbind();
		$("#jdAlertCancel").unbind();

		// 设置title
		if (data.title != null) {
			$("#jdAlertTitle").html(data.title);
		}
		// 设置content
		if (data.content != null) {
			$("#jdAlertContent").html(data.content);
		}
		// 设置确定按钮文本
		if (data.acceptText != null) {
			$("#jdAlertAccept").html(data.acceptText);
		}
		// 取消文本
		if (data.cancelText != null) {
			$("#jdAlertCancel").html(data.cancelText);
		}
		var mode = 0;
		if (data.accept != null) {
			mode++;
		...
		if (mode != 1 && mode != 2) {
			showAlert("警告!!!", "调用alert的数据项不合法");
		} else {
			if (mode == 1) {// 单一回调情况下，取消按钮不显示
				$("#jdAlertCancel").hide();
				$("#jdAlertAccept").click(function() {
					$("#jdAlertDimissType").attr("value", "accept");
					modalObj.modal('hide');
				});
				modalObj.on("hidden.bs.modal", function() {
					$("#jdAlertIsOnShow").attr("value", "hide");
					if (data.accept != null) {
						data.accept(true);
					} else {
						data.cancel(false);
				...
				modalObj.on("hidden.bs.modal", function() {
					$("#jdAlertIsOnShow").attr("value", "hide");
					if ($("#jdAlertDimissType").val() == "cancel") {
						data.cancel(false);
					}else{
						data.accept(true);
					}
			...
			modalObj.modal('show');
			$("#jdAlertIsOnShow").attr("value", "show");
		}
	};
	// ========name space ： al =======================
	al.alert = showAlert;
	al.confirm = showConfirm;
	al.showOptions = showOptions;
	jq.al = al;

	// ==============将alert绑定到jquery对象的“根space”下 =================
	// 这样之前的alert代码就不需要进行更改已适应插件变动了
	// jq.alert=showAlert;
	// jq.confirm=showConfirm;
	// jq.showOptions=showOptions;
